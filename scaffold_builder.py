...
def run():
    ...
    if ("-r" not in userParameters):
        ...
        os.system(which("nucmer")+" "+parameters["-r"]+" "+parameters["-q"]+" --maxmatch --nosimplify"+" && "+which("show-coords")+" out.delta >"+parameters["-p"]+".coords && rm out.delta && mkdir "+parameters["-p"]+"_overlap_alignment")
        return True
